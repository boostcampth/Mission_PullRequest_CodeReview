...
public class MainActivity extends BaseActivity<ActivityMovieBinding, MainPresenter> implements MainContract.View {
     ...
     */
    @Override
    public void startMovieDetailPage(String movieStory, int position) {
        if(movieStory != null) {
            if (movieStory.equals("")) {
                movieStory = getString(R.string.story_result_empty);
            }
        }
        adapter.updateItem(position, movieStory);
    }
}
